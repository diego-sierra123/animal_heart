...
<script>
    $(document).ready(function() {
        ...
        $("#borrar_contenido").on("click", function(){
            $("#search-input").val("");
        })

        let timeoutId;

        // Búsqueda cuando se envía el formulario
        ...
        function realizarBusqueda() {
            ...
            // Si el término está vacío, recargar la página para mostrar todos
            if (searchTerm.trim() === '') {
                pagination.show();
                location.reload();
                return;
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $('#table-container table tbody').html(
                        '<tr><td colspan="6" class="text-center">Buscando...</td></tr>'
                    );
                },
                success: function(response) {
                    // ¡IMPORTANTE! Solo actualizar el tbody, no toda la tabla
                    $('#table-container table tbody').html(response);
                },
                error: function() {
                    $('#table-container table tbody').html(
                        '<tr><td colspan="6" class="text-center">Error en la búsqueda</td></tr>'
                    );
                }
            });
        }
    ...
    document.getElementById('btnActualizar').addEventListener('click', function(e) {
        if (botonPresionado) {
            e.preventDefault();
            return false; // No hace nada si ya fue presionado
        }

        botonPresionado = true;
        // El formulario se envía normalmente solo la primera vez
    });
</script>
